...
static cell AMX_NATIVE_CALL n_WP_Hash(AMX *amx, cell *params)
{
	CHECK_PARAMS(3, "WP_Hash");
	...
	// Get our string to be hashed
	// I doubt they'll EVER be more than 4096, but code it right anyway
	amx_GetAddr(amx, params[3], &amx_cstr);
	amx_StrLen(amx_cstr, &amx_length);
	
	// This code could go in a single loop
	// But this should be more efficient
	while (amx_length > 4096)
	{
		amx_GetString(buffer, amx_cstr, 0, 4096);
		amx_cstr += 4096; // Pointer addition
		amx_length -= 4096;
		NESSIEadd((unsigned char *)buffer, 4096 * 8, &whirlpool_ctx);
	}
	amx_GetString(buffer, amx_cstr, 0, amx_length + 1);
	NESSIEadd((unsigned char *)buffer, amx_length * 8, &whirlpool_ctx);
...
{
	return amx_Register(amx, WPNatives, -1);
}
